...
- 实体初始化逻辑投递至主线程执行，兼容C2ME等异步区块加载模组
- 胸腔类型回退注册添加synchronized保护，防止并发写入损坏
- 血液过滤周期偏移改用ThreadLocalRandom，避免工作线程访问非线程安全的随机源

## 1.9.0 新增玄武岩器官系列与属性显示完善

- 新增玄武岩器官系列，提供火焰抗性与熔岩游泳速度加成，附带水虚弱效果
- 新增玄武岩胸腔类型，猪灵蛮兵改为玄武岩类型
- 新增炽足兽肾脏，提供火焰抗性与熔岩行者加成，附带水过敏效果
- 新增炽足兽胸腔类型
- 抗火生物器官系列新增熔岩游泳速度加成
- 新增共生肠道，食用花朵获得对应迷之炖菜药水效果
- 新增巫蛊之肝，清除自身所有药水效果
- 炼金腺新增解毒属性
- 新增女巫与哞菇胸腔类型，女巫与哞菇改为对应类型
- 新增村民变女巫、哞菇变牛、猪灵僵尸化等转换的器官转换映射
- 新增熔岩游泳速度属性，提升在熔岩中的移动速度
- 新增熔岩行者属性，允许在熔岩表面行走，潜行时失效
- 新增水虚弱属性，在水中降低挖掘速度与攻击伤害
- 火焰抗性达到阈值时清除熔岩迷雾并隐藏火焰屏幕覆盖层
- 熔岩中跳跃与下沉受熔岩游泳速度加成
- 熔岩中疾跑水平衰减对齐水中机制，免疫时按熔岩游泳速度减缓额外下沉重力
- 力量与速度属性缩放系数统一并下调，平缓属性堆叠收益曲线
- 盐水型肌肉游泳速度加成上调
- 补全游泳速度、幸运、击退抗性、重力、交互距离等原版属性的属性显示
...
- 新增劫掠兽专属胸腔类型
- 补全节肢生物、海龟、青蛙、兔子、史莱姆、岩浆怪的胸腔类型回退检测
- 修复跨维度重生时 JEI 客户端缓存未清理的问题
- 手术附魔改为附魔台可获取
...
## 1.8.10 新增生物胸腔类型与效果同步修复

- 新增力量型动物（北极熊、嗅探兽）、疣猪兽、速度型动物（猫、豹猫）、弹跳型动物（山羊）的胸腔类型
- 修复有害效果时长调整在客户端重复执行，导致效果时长显示错误的问题

## 1.8.9 器官属性平衡调整与效果同步修复

- 调整抗火、末影、盐水等器官系列属性数值
- 部分器官新增属性（蛛丝腺交互范围、毒腺解毒、活性烈焰棒力量等）
- 修复效果同步在效果尚未添加到实体时误触发的问题
- 解毒免疫持续时间阈值默认值上调至 20 tick

## 1.8.8 属性缩放公式调整与解毒免疫阈值

- 新增平方根反比缩放算法，正数区间衰减曲线更平缓，缓解高属性堆叠下的收益递减惩罚
- 解毒效率、凋零化、耐力、肺活量的缩放统一由指数反比改为平方根反比
- 新增配置项「解毒免疫持续时间阈值」，缩减后有害效果持续时长不超过此值时直接免疫
- 解毒/凋零化效果处理跳过瞬时效果
- 凋零化属性显示新增「持续时间%」字段
- 解毒效率描述补充免疫阈值说明
- 雪之核心新增 5 点冰霜抗性
- JEI 胸腔类型页面实体由刷怪蛋图标改为 3D 实体模型渲染，按碰撞箱自适应缩放并跟随鼠标转头
- JEI 胸腔页面悬停器官槽位时显示完整器官 tooltip，复用页面虚拟实体生成胸腔数据
- 开胸器现在可以在铁砧上合并附魔
- 胸腔类型注册表改用 LinkedHashMap，保持注册顺序稳定
- 食草动物瘤胃技能描述细化，区分玩家（饥饿值/饱和度）与生物（回血）的恢复效果数值
- 盔甲架胸腔类型标记为不需要呼吸和健康

## 1.8.7 熔炉内核降级转换逻辑修复

- 修复熔炉内核技能降级时「转换后时长已达上限则免费降级」的问题，降级必须消耗1个燃料才生效
- 降级时首次消耗燃料允许总时长超过上限，但不超过上限的1.5倍

## 1.8.6 动态属性刷新完善，熔炉内核与食草动物瘤胃调整

- 运行时器官变更启用多轮迭代刷新动态器官属性，modifier 收敛检测改用浮点容差比较
- 调整熔炉内核技能效果持续时间换算，按等级折算保持总收益守恒
- 铁器官烧炼产物由8铁粒改为1铁锭
- 食草动物瘤胃Goal技能回血量接入消化与营养属性体系，消化能力不足时不触发
- `refreshOnOrganChange` 重命名为 `refreshDynamicAttribute`，属性刷新职责移至框架层

## 1.8.5 远古器官属性平衡调整

- 调整远古器官属性数值（多数由 2 上调为 3，部分特殊调整）
- 守卫者之眼与远古守卫者之眼交互范围属性上调
...
- 新增复活安全兜底：玩家复活后若缺失关键器官（心脏/肺/脊柱/肾脏），自动优先填充空槽，空槽不足时按优先级替换非关键器官，避免反复死亡循环
- 熔炉内核主动技能新增动态公式展示（按 Ctrl 展开「200÷等级÷20」），同时熔炉之力属性 tooltip 改为显示恢复间隔、效果等级、最大持续时间
- 显示等级统一改用 amplifier+1，与游戏内效果等级（如「中毒 I」对应「Lv1」）保持一致
- 修复 `/chestcavity` 命令传入无效 size 时被静默回退到默认值的问题，改为抛出「无效的胸腔容量大小」错误
- 术后缝合附魔触发回血后，对开胸器物品施加 1 秒冷却，防止连续触发刷血

## 1.8.2 新增手术附魔与安全手术修复

- 新增麻醉手术附魔：成功开胸时对目标施加 3 秒缓慢效果，等级等于附魔等级
- 新增术后缝合附魔：关闭胸腔界面时，每级为目标恢复 1 点生命值
- 修复安全手术附魔在生效的情况下，仍会对宠物开胸的 bug
- 调整安全手术拦截优先级，使其在生命值检查和液压钳逻辑之前生效
- 修复开胸伤害被攻击者增伤器官异常放大导致杀死目标生物的问题，开胸伤害现在跳过攻击者器官的 attack 回调
- 修复 `ChestCavityData` 初始化标记被反序列化无条件置为 true 的问题，改为持久化保存并在老存档（≤1.8.1）兜底为 true
